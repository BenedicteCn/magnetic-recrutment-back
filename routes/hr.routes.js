...
router.post('/hr/signup', async (req, res, next) => {
  try {
    const { username, email, password } = req.body;
    const foundUser = await HR.findOne({ username });
    const regex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    ...
    const emailMessage = await transporter.sendMail({
      ...
      to: email,
      subject: 'Welcome!',
      text: 'Hello Recuiter ! Welcome to Magnetic',
    });

    console.log(emailMessage);

    res.status(201).json({
      message: 'Hello Recuiter ! Welcome to Magnetic ! ',
      createdUser,
    });
  } catch (error) {
    console.log(error);
...
router.patch('/hr/password/', isAuthenticated, async (req, res, next) => {
  try {
    ...
    if (isPasswordMatched) {
      ...
      const updatedUser = await HR.findByIdAndUpdate(req.user._id, {
        password: hashedPassword,
      });
